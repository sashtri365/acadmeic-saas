...
type FormState = "idle" | "submitting" | "invalid" | "unavailable" | "reset";
...
export default function LoginForm({ tenantSlug }: LoginFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFormState(result.requiresPasswordReset ? "reset" : "idle");
  ...
  const message = formState === "invalid"
    ...
    : formState === "unavailable"
      ...
      : formState === "reset"
        ? "Your institution requires a password reset before you continue."
        : "";
  ...
  return (
    <form className="auth-card" onSubmit={handleSubmit}>
      ...
      <div className="auth-options"><label className="remember-option"><input type="checkbox" /> Keep me signed in</label><button className="forgot-link" type="button">Forgot password?</button></div>
      {message && <p className={`form-message ${formState === "reset" ? "form-message-info" : ""}`} role="alert">{message}</p>}
